...
\section{Introduction}
  ...
  Especially for the rendering of volumetric datasets in medicine, it is a valuable tool to reduce a vast amount of complex information to its essence given a focus and a context.
  % Especially for the rendering of volumetric datasets in medicine, scanned artifacts in archaeology, or technical construction designs in engineering and manufacturing, it is a valuable tool to reduce a rich set of complex information to its essence regarding focus and context for better understanding.
  % Evolving out of scientific illustration, it is a subfield of scientific visualization.
  It is well-known that photorealism is not really able to provide an efficient depiction of features of interest.
  Our knowledge of human cognition shows that, artistic drawings or paintings seem to be more suitable for communication and more pleasing in visual experience in comparison to a photograph of the same scene.
  Therefore non-photorealistic rendering (NPR) techniques, typically inspired by artistic styles, are used to create such illustrations.
  NPR is able to show features that may not be seen by using a global illumination algorithm.
  % But in contrast to visual art, scientific visualization needs depicting features of abstract data in an expressive way, while simultaneously providing insight into the underlying data.

  One of the most important tools used in illustrative visualization for conveying the shape of an object are feature lines.
  By taking advantage of human visual acuity, line drawings can express a large amount of information which makes them more expressive than photographs.
  There are many different types of commonly-used feature lines, such as contours (silhouettes), suggestive contours, or ridge-valley lines.
  Typically, they are only dependent on the surface geometry, such as normal and curvature, and/or the view position.
  However, human perception is highly sensitive to high luminance variations.
  As a consequence, conveying the shape of objects by using feature lines should also depend on the illumination of an object.

  In this report we present photic extremum lines (PELs) created by.
  ...
  This enables further processing such as line stylization.
